...
class DDCEditClientInfoModelFactory: NSObject {
    class func integrateData(model: DDCCustomerModel, channels:[DDCChannelModel]?) -> [DDCContractInfoViewModel] {
        ...
        let gender: DDCContractInfoViewModel = DDCContractInfoViewModel.init(title: "性别", placeholder: "请选择性别", text: sex, isRequired: true, tag: 0)
        //生日
        ...
        //年龄
        let age: DDCContractInfoViewModel = DDCContractInfoViewModel.init(title: "年龄", placeholder: "年龄", text: model.age ?? "", isRequired: false, tag: 0)
        //邮箱
        let email: DDCContractInfoViewModel = DDCContractInfoViewModel.init(title: "邮箱", placeholder: "邮箱", text: model.email ?? "", isRequired: false, tag: 0)
        //职业
        let careerText: String = model.career != nil ? DDCContract.occupationArray[Int(model.career!)!] : ""
        let career: DDCContractInfoViewModel = DDCContractInfoViewModel.init(title: "职业", placeholder: "请选择职业", text: careerText, isRequired: false, tag: 0)
        //渠道
